...
from app_modules.command_parser import Commander, App_Modes
...
class InputHandler:
    ...
    def print_commands(self) -> None:
        ...
        print(f"{App_Modes.DELETE_EMPTYLIKE_DIRS.value}: delete empty-like folders")
        print(f"{App_Modes.EXTRACT_ARCHIVES.value}: extract archives")
        print(
            f"{App_Modes.DELETE_UNPACKED_ARCHIVES.value}: delete already unpacked archives"
        )
        print(
            f"{App_Modes.REMOVE_NESTED_DIRECTORIES.value}: remove nested folders after unpacking"
        )
